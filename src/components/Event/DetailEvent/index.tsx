  ...
  return (
    <>
      <div className="relative top-0 h-[400px] md:h-screen flex flex-col justify-center">
        <Image
          loader={() => myLoader(eventData?.img?.data?.attributes?.url || "")}
          src={`${process.env.NEXT_PUBLIC_API_URL_FILE}${eventData?.img?.data?.attributes?.url}`}
          layout="fill"
          objectFit="cover"
          alt="Home"
        />
        ...
        <div className="flex flex-col  mt-20 gap-8 lg:flex-row">
          <div className="flex-1 flex flex-col gap-10">
            <div className="flex flex-col gap-4 items-center lg:flex-row">
              <Image
                loader={() =>
                  myLoader(eventData?.img?.data?.attributes?.url || "")
                }
                src={`${process.env.NEXT_PUBLIC_API_URL_FILE}${eventData?.img?.data?.attributes?.url}`}
                width={200}
                height={300}
